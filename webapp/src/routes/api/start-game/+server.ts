...
export const POST: RequestHandler = async ({ request, fetch }) => {
	...
	try {
		...
		const { success: generateSuccess, error: generateError } = await generateCards(
			category,
			game,
			firstCardFetch,
			fetch
		...
		const { success: assigningSuccess, error: assigningError } = await assigningCards(
			game.id,
			players
		);
	...
	try {
		...
		for (let i = 0; i < players.length; i++) {
			...
			if (updateCardsCountError) {
				throw new Error(updateCardsCountError.message);
			}
		}
